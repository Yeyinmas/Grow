...
5. **配置 Game Manager**：
   ...
   - 道具配置：`Bush Count = 2`、`Trap Count = 1`、`Bush Grow Turns = 2`、`Trap Hold Turns = 2`，可按需调整。
...
- **回合**：玩家走一步 → 怪物依次走一步（每个怪物一步）→ 回合数 +1。
- **玩家**：不能原地等待，每回合必须移动一步；移动后立即结算。
- **被困**：玩家四周没有可走的格子时，判定失败。
- **被抓**：怪物走到玩家所在格，判定失败。
- **终点**：玩家走到 `F` 格，立即胜利，无需等怪物行动。
- **道具一（灌木丛）**：站在 `D` 格按 `1` 放置，`Bush Grow Turns` 回合后该格变成障碍堵路。
- **道具二（陷阱）**：站在 `D` 格按 `2` 放置，怪物踩入后被困 `Trap Hold Turns` 回合，陷阱消耗。
- **开关/门**：玩家踩到 `S` 格，所有 `G` 门打开（变为可通行）。
- **怪物 AI**：每回合用 BFS 求到玩家的最短路径走一步；多条最短路径时按「上、下、左、右」固定顺序选择，结果可预测。可开启 `Show Monster Preview` 显示怪物下一步位置。
...
- 生长规则：下一格**必须是道路（`R`）**，且不能被玩家、怪物占用，否则该回合不生长（下回合继续尝试）。
